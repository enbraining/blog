...
- **Blog** powered by [Velite](https://velite.js.org/) and MDX: write posts in `content/*.mdx` with frontmatter; they are type-safe and built at compile time.
...
| Content    | Velite, MDX, remark-gfm, rehype-pretty-code, shiki |
...
├── velite.config.ts             # Velite posts schema and MDX compile configuration
├── next.config.mjs              # Velite build hook and security headers
├── eslint.config.mjs
├── tailwind.config.ts
├── tsconfig.json
└── package.json
...
- **Blog posts:** `content/*.mdx` (title, dates, body)
...
- **Blog:** `import { posts } from "@/.velite"` in `app/blog/page.tsx` and `app/blog/[slug]/page.tsx`.
...
Blog posts live in `content/` as `.mdx` files. Each file must have this frontmatter (enforced by `velite.config.ts`):
...
After adding or editing a post, run `npm run content` (or rely on dev) so Velite regenerates the content data; the blog index and `/blog/[slug]` will update. Slugs are derived from the file path (e.g. `content/api-design-principles.mdx` → `/blog/api-design-principles`).
...
- **Next.js:** `next.config.mjs` – `reactStrictMode: true`, Velite build hook, security headers.
- **Velite:** `velite.config.ts` – posts collection, `content` directory, title/date schema, and the `remarkCodeMeta` transform.
- **ESLint:** `eslint.config.mjs` – Next.js core-web-vitals and global ignores (`.next`, `out`, `build`, `.velite`).
...
| `npm run content`  | Generate the Velite content data.                              |
...
Portfolio, personal website, Next.js, React, TypeScript, Tailwind CSS, App Router, MDX, Velite, blog, Open Graph, SEO, Radix UI, dark mode, responsive, Vercel, static site, SSG, developer portfolio, resume, projects, hackathons.

---

## Conclusion

This project is a complete portfolio + blog template with no backend or environment variables required. You get a modern stack (Next.js 16, React 19, TypeScript, Tailwind), type-safe content (Velite + MDX), good SEO and OG images, and reusable sections and UI. Use it to learn the App Router, metadata, and content pipelines, or as a base for your own portfolio and blog.
